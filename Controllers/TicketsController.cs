...
using static NLPHelpDesk.Helpers.CsvHelper;
...
public class TicketsController : Controller
{
    ...
    public async Task<IActionResult> Create(Ticket newTicket)
    {
        ...
        try
        {
            ...
            ProcessTicketInBackground(newTicket, userId, role);
    ...
    /// <summary>
    /// Processes a ticket in the background, including category and priority prediction, and user assignment.
    /// </summary>
    /// <param name="ticket">The Ticket object to process.</param>
    /// <param name="userId">The ID of the user who created the ticket.</param>
    /// <param name="role">The role of the user who created the ticket.</param>
    /// <returns>A Task representing the asynchronous operation.</returns>
    private async Task ProcessTicketInBackground(Ticket ticket, string userId, string role)
    {
        // Validate the ticket object.
        if (ticket == null)
        {
            _logger.LogWarning("Invalid ticket object");
            return;
        }
        
        // Queue the background task.
        // Keeping your web application responsive.
        // Long-running operations should never be performed directly in the controller action.
        _backgroundTaskQueue.QueueBackgroundWorkItem(async (provider) =>
        {
            // Create input data for the prediction services
            CsvData inputData = new CsvData
            {
                // Combine title and description
                Text = ticket.TicketTitle + "," + ticket.TicketDescription
            };
            
            // Read any necessary CSV data
            ...
            var scopedContext = scopedProvider.ServiceProvider.GetRequiredService<ApplicationContext>();
            var categoryPredictionService = scopedProvider.ServiceProvider.GetRequiredService<ICategoryPredictionService>();
            var priorityPredictionService = scopedProvider.ServiceProvider.GetRequiredService<IPriorityPredictionService>();
            var helpDeskCategoryService = scopedProvider.ServiceProvider.GetRequiredService<IHelpDeskCategoryService>();
            var ticketService = scopedProvider.ServiceProvider.GetRequiredService<ITicketService>();
            
            // Perform category prediction.
            var categoryPrediction = await categoryPredictionService.GetCategoryPrediction(data, inputData);
            
            // Call the service to get the category details from the database.
            var category = await helpDeskCategoryService.GetHelpDeskCategory(scopedContext, categoryPrediction.PredictedCategory.ToString());
            
            // Perform priority prediction.
            var priorityPrediction = await priorityPredictionService.GetPriorityPrediction(data, inputData);
            
            // Parse the predicted priority.
            var priority = Enum.TryParse<Priority>(priorityPrediction.PredictedPriority, true, out var priorityEnum)
                ? priorityEnum
                : Priority.Low;
            
            // Update the ticket with the predicted category and priority.
            var updateResult = await ticketService.UpdateTicket(scopedContext, ticket.TicketId, category.CategoryId, priority);

            // Handle user assignment based on role after successful ticket update.
            if (updateResult)
            {
                // Chekc user's role
                if (role == ROLE_TECHNICIAN)
                {
                    // Assign login user(technician) to this ticket
                    var userTicketResult = await ticketService.CreateUserTicket(ticket.TicketId, userId);
                    if (!userTicketResult)
                    {
                        // Log error message
                        _logger.LogError("Failed to assign user to this ticket");
                    }
                }
                else
                {
                    // NOT assign anyone to this ticket at this time.
                    var userTicketResult = await ticketService.CreateUserTicket(ticket.TicketId);
                    if (!userTicketResult)
                    {
                        // Log error message
                        _logger.LogError("Failed to assign user to this ticket");
            ...
            {
                // Include ticketId in log
                _logger.LogError("Failed to identify a help desk category");
            }
        });
    }
